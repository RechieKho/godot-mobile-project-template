...
func play(position: Vector2, size: float, duration := 0.7 , force := 0.1, thickness := .0, backward := false):
	distortion_mark.global_position = position
	distortion_force = force if not backward else 0
	distortion_size = 0 if not backward else size
	distortion_thickness = thickness if not backward else 0
	is_playing = true
	await create_tween().set_parallel(true) \
		.tween_property(self, "distortion_force", 0 if not backward else force, duration) \
		.tween_property(self, "distortion_size", size if not backward else 0, duration) \
		.tween_property(self, "distortion_thickness", 0 if not backward else thickness, duration)
	is_playing = false
